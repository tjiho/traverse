...
def rerank(query: str, candidates: list[str], tags_data, category_searched: list[str], top_k: int = 5) -> list[tuple[str, float]]:

    filtered_candidates = [tag for tag in candidates if get_category(tag, tags_data) in category_searched]
